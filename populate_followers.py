...
from backend.models import FollowerHistory, User
...
print("--- INICIANDO IMPORTAÇÃO DE SEGUIDORES ---")
...
for line in lines:
    ...
    try:
        ...
        exists = db.query(FollowerHistory).filter(
            FollowerHistory.date == final_date,
            FollowerHistory.platform == 'tiktok'
        ).first()
        
        if not exists:
            new_record = FollowerHistory(user_id=user_id, platform='tiktok', count=followers, date=final_date)
            db.add(new_record)
            count_saved += 1
...
db.commit()
db.close()
print(f"--- SUCESSO: {count_saved} REGISTROS IMPORTADOS! ---")